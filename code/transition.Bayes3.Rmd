...
# Some notes
Hour = 13196 Day 550 is wonky. All reps have counts that are about an order of magnitude low. 
Also, there's a break in sampling from 13292 - 14108 hours (554 -588 days); this likely corresponds with Brent being out of lab for survery. 
Perhaps for these reasons, it's reasonable to stop day 548?
...
```{r}
png(filename="/Users/lennonj/GitHub/SporMut/output/pop.densities.D.BayesIII.png",
...
axis(side = 1, lwd.ticks = 2, cex.axis = 1.25,
     at = c(0, 100, 200, 300, 400, 500, 600, 700), labels = T)

axis(side = 3, lwd.ticks = 2, cex.axis = 1.25,
     at = c(0, 100, 200, 300, 400, 500, 600, 700), labels = F)
...
curve(coef(fit2)[[2]] + (coef(fit2)[[1]] - coef(fit2)[[2]]) / 
      ...
      from = -6, to = 650, add = TRUE, lty = 2, lwd = 2.5, col = "black") 
...
curve(coef(fit3)[[2]] + (coef(fit3)[[1]] - coef(fit3)[[2]]) / 
      ...
      from = 0, to = 650, add = TRUE, lty = 2, lwd = 2.5, col = "black") 
...
# Show Plot
img <- readPNG("/Users/lennonj/GitHub/SporMut/output/pop.densities.D.BayesIII.png")
grid.raster(img)
```