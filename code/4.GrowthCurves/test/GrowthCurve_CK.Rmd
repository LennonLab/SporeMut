...
## To skip followint chunks, read this data 
```{r}
...
umaxplot <- ggplot(plt_qnts_dat, aes(y = clones, x = plt_qnts_dat[,2], group = clones))+
  geom_vline(xintercept = 1, linetype = "dashed")+
...
ggsave("umaxJags.pdf", umaxplot, width = 5, height = 6)
...
jdatK<-list("M_mu"=as.data.frame(sapply(dat,"[",4))[,-1],"An_mu"=dat[['anc']]$A,"wts_m"=wts_matK[,2:11],"wts_a"=wts_matK[,1])
...
plt_qntsK<-matrix(0,10,3)
rownames(plt_qntsK)<-clns[-1]
colnames(plt_qntsK)<-c("2.5%","50%","97.5%")
for(i in 1:10)plt_qntsK[i,]<-quantile(exp(valsK[,i+1])/exp(valsK[,1]),c(.025,.5,.975))

#plot the results 
plt_qnts_datK <- as.data.frame(plt_qntsK)
plt_qnts_datK$clones <- rownames(plt_qnts_datK)

plt_qnts_datK$clones <- factor(plt_qnts_datK$clones, c("M4", "M13", "M79", "M17", "M19", "M21", "M41", "M54", "M23", "M26"))

Kplot <- ggplot(plt_qnts_datK, aes(y = clones, x = plt_qnts_datK[,2], group = clones)) +
  geom_vline(xintercept = 1, linetype = "dashed")+
...
jdat.lag<-list("M_mu"=as.data.frame(sapply(dat,"[",8))[,-1],"An_mu"=dat[['anc']]$L,"wts_m"=wts_mat.lag[,2:11],"wts_a"=wts_mat.lag[,1])
...
plt_qnts.lag<-matrix(0,10,3)
rownames(plt_qnts.lag)<-clns[-1]
colnames(plt_qnts.lag)<-c("2.5%","50%","97.5%")
for(i in 1:10)plt_qnts.lag[i,]<-quantile(exp(vals.lag[,i+1])/exp(vals.lag[,1]),c(.025,.5,.975))

#plot the results 
plt_qnts_dat.lag <- as.data.frame(plt_qnts.lag)
plt_qnts_dat.lag$clones <- rownames(plt_qnts_dat.lag)

plt_qnts_dat.lag$clones <- factor(plt_qnts_dat.lag$clones, c("M4", "M13", "M79", "M17", "M19", "M21", "M41", "M54", "M23", "M26"))
