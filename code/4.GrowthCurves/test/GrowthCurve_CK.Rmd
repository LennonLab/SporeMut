...
treats_param_long <- treats_param %>%
  select(b0, A, umax, L, evo.type, cell.type, mutation, clone) %>%
...
treats_param_se <- treats_param %>%
  select(b0.se, A.se, umax.se, L.se, mutation) %>%
...
# Stats - Canan
```{r}
mod1 <- aov(A ~ mutation+Error(clone), data = treats_param)
summary(mod1)
emm_mut1 <- emmeans(mod1, "mutation", weights = "cells")
letter1 <- cld(emm_mut1, Letter = letters)

mod2 <- aov(b0 ~ mutation+Error(clone), data = treats_param)
summary(mod2)
emm_mut2 <- emmeans(mod2, "mutation", weights = "cells")
letter2 <- cld(emm_mut2, Letter = letters)

mod3 <- aov(L ~ mutation+Error(clone), data = treats_param)
summary(mod3)
emm_mut3 <- emmeans(mod3, "mutation", weights = "cells")
letter3 <- cld(emm_mut3, Letter = letters)

mod4 <- aov(umax ~ mutation+Error(clone), data = treats_param)
summary(mod4)
emm_mut4 <- emmeans(mod4, "mutation", weights = "cells")
letter4 <- cld(emm_mut4, Letter = letters)

dataLetter <-c(letter1$.group, letter2$.group, letter3$.group, letter4$.group)

treats_param_letter <- treats_param_sum %>%
  bind_cols(letters = gsub(" ", "", dataLetter) )

ggplot(treats_param_letter, aes(x = mutation, y = meanvalue, color = mutation))+
  geom_jitter(data = treats_param_long, aes(x = mutation, y = value), alpha = .3)+
  geom_point(aes(x = mutation, y = meanvalue), size = 5, shape = 1, stroke = 1)+
  facet_wrap(~parameter, scales = "free")+
  geom_errorbar(aes(ymin = (meanvalue - meanerror), 
  ...
  theme(strip.text.x = element_text(size = 14))+
  scale_color_manual(values = c("#E64B35","#4DBBD5","#3C5488","#F39B7F"))+
  geom_text(aes(label = letters, y = meanvalue + meanerror), vjust = -0.5, color = "black", size = 5, hjust = 0.5)
