...
biofilmplot <- ggplot(plt_qnts_dat, aes(y = clones, x = plt_qnts_dat[,2], group = clones))+
  ...
  geom_errorbar(aes(xmin =plt_qnts_dat[,1], xmax =plt_qnts_dat[,3]), width = 0, linewidth =1, color = "darkblue")+
  mytheme+
...
data_long <- data_df_rel %>%
  pivot_longer(cols = m4:m79, names_to = "clones", values_to = "dists")
...
dist.bio.rel <- ggplot(data_long, aes(y = as.numeric(clones), x = dists, group = clones, 
  ...
  stat_density_ridges(geom = "density_ridges_gradient", calc_ecdf = TRUE, 
                      rel_min_height = 0.005, alpha = 0.5, scale = 1, color = "grey25" )+
  xlab("Relative biofilm")+
  scale_x_continuous(limits = c(0, 20), sec.axis=dup_axis())+
  mytheme+
...
ggsave("biofilm_jags_dist.pdf", dist.bio.rel, width = 5, height = 5)
